...
module.exports = function(app, config, passport, assets) {
	...
	// in production we have a reverse proxy that compresses
	if (env == 'development') {
	...
	app.configure(function() {
		...
		app.use(express.session({
			secret: config.get().vpdb.secret,
			store: new mongoStore({
				url: config.get().vpdb.db,
				collection: 'sessions'
			})
		}));
