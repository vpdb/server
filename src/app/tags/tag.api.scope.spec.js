...
describe('The scopes of the `Tag` API', function() {
	...
	describe('using a service token', function() {
		...
		it('should deny access to tag creation', done => {
			request.post('/api/v1/tags').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to tag deletion', done => {
			request.del('/api/v1/tags/1234').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
	});
});