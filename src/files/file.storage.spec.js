...
describe.only('The VPDB `file` API', () => {
	...
	describe('when deleting a file', () => {
		...
		// it.skip('should succeed as owner of the file', async () => {
		// 	const user = 'member';
		//
		// 	// 1. upload
		// 	res = await api.onStorage()
		...
		//
		// 	// 2. check it's there
		// 	await api.as(user).get(ApiClient.urlPath(url)).then(res => res.expectStatus(200));
		//
		// 	// 3. delete
		// 	await api.as(user).del('/v1/files/' + id).then(res => res.expectStatus(204));
		//
		// 	// 4. check it's not there
		// 	await api.as(user).get(ApiClient.urlPath(url)).then(res => res.expectStatus(404));
		// });

		// it.only('should fail if not owner of the file', async () => {
		// 	const user = 'member';
		// 	// 1. upload
		// 	res = await api.onStorage()
		// 		.as(user)
		...
		// 	expect(res.data.url).to.be.ok();
		//
		// 	await api.as('anothermember').del('/v1/files/' + res.data.id).then(res => res.expectError(404));
		// });
		//
