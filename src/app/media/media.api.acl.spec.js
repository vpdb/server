...
describe('The ACLs of the `Media` API', function() {
	...
	describe('for anonymous clients', function() {
		...
		it('should deny access to deleting media', function(done) {
			request.del('/api/v1/media/1234').end(hlp.status(401, done));
		});
	...
	describe('for logged clients (role member)', function() {
		...
		it('should allow access to deleting media', function(done) {
			request.del('/api/v1/media/1234').as('member').end(hlp.status(404, done));
		});
