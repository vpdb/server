...
describe('The scopes of the `Game` API', function() {
	...
	describe('using a service token', function() {
		...
		it('should deny access to game creation', done => {
			request.post('/api/v1/games').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to game update', done => {
			request.patch('/api/v1/games/1234').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to game deletion', done => {
			request.del('/api/v1/games/1234').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});

		it('should deny access to game backglass creation through game', done => {
			request.post('/api/v1/games/1234/backglasses').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to game release name generation', done => {
			request.get('/api/v1/games/1234/release-name').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
	});
});