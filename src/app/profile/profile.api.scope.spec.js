...
describe('The scopes of the `Profile` API', function() {
	...
	describe('using an "all" token', function() {
		...
		it('should allow access to user profile', done => {
			request.get('/api/v1/profile').with(tokenAll).end(hlp.status(200, done));
		});
		it('should allow access to user profile update', done => {
			request.patch('/api/v1/profile').send({}).with(tokenAll).end(hlp.status(200, done));
		});
		it('should allow access to user log retrieval', done => {
			request.get('/api/v1/profile/logs').with(tokenAll).end(hlp.status(200, done));
		});
	});
	...
	describe('using a login token', function() {
		...
		it('should deny access to user profile', done => {
			request.get('/api/v1/profile').with(tokenLogin).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to user profile update', done => {
			request.patch('/api/v1/profile').send({}).with(tokenLogin).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to user log retrieval', done => {
			request.get('/api/v1/profile/logs').with(tokenLogin).end(hlp.status(401, 'invalid scope', done));
		});
	});
	...
	describe('using a community token', function() {
		...
		it('should allow access to user profile', done => {
			request.get('/api/v1/profile').with(tokenCommunity).end(hlp.status(200, done));
		});
		it('should deny access to user profile update', done => {
			request.patch('/api/v1/profile').send({}).with(tokenCommunity).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to user log retrieval', done => {
			request.get('/api/v1/profile/logs').with(tokenCommunity).end(hlp.status(401, 'invalid scope', done));
		});
	});
	...
	describe('using a service token', function() {
		...
		it('should deny access to user profile', done => {
			request.get('/api/v1/profile').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to user profile update', done => {
			request.patch('/api/v1/profile').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to user log retrieval', done => {
			request.get('/api/v1/profile/logs').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
	});
});