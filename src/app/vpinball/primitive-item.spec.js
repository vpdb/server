...
describe.only('The VPinball primitive generator', () => {
	...
	it('should generate a compressed mesh', async () => {
		const vpxFile = await api.fileHelper.createVpx('member', 'table-sink.vpx');
		const gltf = await three.loadGlb('member', vpxFile.variations.gltf);
		const sinkMesh = three.find(gltf, 'primitives', 'primitive-Primitive1');
		const sinkMeshVertices = sinkMesh.geometry.attributes.position;
		const expectedVertices = [
		...
		three.expectVerticesInArray(expectedVertices, sinkMeshVertices.array);
	});
