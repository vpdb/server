...
describe('The VPinball table parser', () => {
	...
	it('should generate the correct table mesh', async () => {
		...
		const table = getTable(gltf);
		const playfieldMesh = table.children.find(c => c.name === 'playfield').children[0];
...
function getTable(gltf) {
	return gltf.scene.children.find(c => c.name === 'playfield');
}
