...
describe('The scopes of the `Media` API', function() {
	...
	describe('using a service token', function() {
		...
		it('should deny access to media creation', done => {
			request.post('/api/v1/media').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to media deletion', done => {
			request.del('/api/v1/media/1234').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
	});
});