...
export function koaWebsiteHandler(root: string, opts?: any) {
	...
	return async function handleWebsite(ctx: Context, next: () => Promise<any>) {
		try {
			...
			if (ctx.path.startsWith(settings.apiInternalPath()) || settings.apiExternalPath() || ctx.path.startsWith(settings.storagePublicPath()) || ctx.path.startsWith(settings.storageProtectedPath())) {
				throw err;
			}
			try {
				if (/\/$|\/[^.]+$/i.test(ctx.path)) {
