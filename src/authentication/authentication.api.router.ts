...
router.post(settings.apiPath('/authenticate'), api.authenticate);
...
if (config.vpdb.passport.github.enabled) {
	...
	router.get(settings.apiPath('/redirect/github'), passport.authenticate('github', { session: false, scope: [ 'user:email' ] }));
}
if (config.vpdb.passport.google.enabled) {
	router.get('/auth/google', passport.authenticate('google', { session: false, scope: 'email' }));
	router.get(settings.apiPath('/redirect/google'), passport.authenticate('google', { session: false, scope: 'email' }));
}
config.vpdb.passport.ipboard.forEach(ipbConfig => {
	if (ipbConfig.enabled) {
		...
		router.get(settings.apiPath('/redirect/' + ipbConfig.id), passport.authenticate(ipbConfig.id, { session: false }));
	}
});

// oauth callback
router.get(settings.apiPath('/authenticate/:strategy'), api.anon(api.user.authenticateOAuth2));