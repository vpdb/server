...
describe('The scopes of the API', function() {
	...
	describe('using a service token', function() {
		...
		it('should deny access to ipdb info retrieval', done => {
			request.get('/api/v1/ipdb/1234').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});

		it('should deny access to roles retrieval', done => {
			request.get('/api/v1/roles').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
	});
});