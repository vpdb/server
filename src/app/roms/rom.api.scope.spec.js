...
describe('The scopes of the `ROM` API', function() {
	...
	describe('using a service token', function() {
		...
		it('should deny access to rom creation', done => {
			request.post('/api/v1/roms').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to rom deletion', done => {
			request.del('/api/v1/roms/1234').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to rom creation through game', done => {
			request.post('/api/v1/games/1234/roms').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
	});
});