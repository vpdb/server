...
describe('The scopes of the `Release` API', function() {
	...
	describe('using a service token', function() {
		...
		it('should deny access to release creation', done => {
			request.post('/api/v1/releases').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to release update', done => {
			request.patch('/api/v1/releases/1234').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to release deletion', done => {
			request.del('/api/v1/releases/1234').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});

		it('should deny access to release version creation', done => {
			request.post('/api/v1/releases/1234/versions').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to release version update', done => {
			request.patch('/api/v1/releases/1234/versions/1.0').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to release file validation', done => {
