...
describe('The scopes of the `Token` API', function() {
	...
	describe('using a service token', function() {
		...
		it('should deny access to token creation', done => {
			request.post('/api/v1/tokens').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to token retrieval', done => {
			request.get('/api/v1/tokens').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to token deletion', done => {
			request.del('/api/v1/tokens/1234').with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
		it('should deny access to token update', done => {
			request.patch('/api/v1/tokens/1234').send({}).with(tokenService).end(hlp.status(401, 'invalid scope', done));
		});
	});
});